...
class NNChatController: NMessengerViewController {
    ...
    func socketMessageEvent() {
        viewModel?.socketMessageEvent(onSuccess: { [weak self] (text) in
            ...
            let textContent = TextContentNode(textMessageString: text, dateString: Date().getStringTime(), currentViewController: vc, bubbleConfiguration: vc.sharedBubbleConfiguration)
