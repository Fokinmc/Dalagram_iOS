...
extension DialogsController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if chatInfo.user_id != 0 { // Single Chat
            let vc = NNChatController(type: .single, info: chatInfo, dialogId: currentDialog.id)
            vc.hidesBottomBarWhenPushed = true
            self.navigationController?.pushViewController(vc, animated: true)
        } else if chatInfo.group_id != 0 { // Group Chat
            let vc = NNChatController(type: .group, info: chatInfo, dialogId: currentDialog.id)
            vc.hidesBottomBarWhenPushed = true
            self.navigationController?.pushViewController(vc, animated: true)
        } else if chatInfo.channel_id != 0 { // Channel
            let vc = NNChatController(type: .channel, info: chatInfo, dialogId: currentDialog.id)
            vc.hidesBottomBarWhenPushed = true
            self.navigationController?.pushViewController(vc, animated: true)
        }
